...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:output omit-xml-declaration="no" encoding="UTF-8"
		...
		doctype-system="http://www.daisy.org/z3986/2005/dtbook-2005-1.dtd" indent="yes" method="xml"/>
	...
	<xsl:template name="copyAtts">
		<xsl:for-each select="@*">
			<xsl:choose>
				...
				<xsl:when test="name() = 'name' and matches(., 'dc:.+')">
					<xsl:attribute name="name">
						<xsl:value-of select="concat('dc:', upper-case(substring(., 4, 1)), substring(., 5))"/>
					</xsl:attribute>
				</xsl:when>
				<xsl:otherwise>
					<xsl:copy-of select="."/>
	...
	<xsl:template match="frontmatter | bodymatter | rearmatter">
		<xsl:element name="{local-name()}" namespace="http://www.daisy.org/z3986/2005/dtbook/">
			...
			<xsl:for-each-group select="*"
				group-adjacent="local-name() = 'level' or local-name() = 'level1' or local-name() = 'doctitle' or local-name() = 'docauthor'">
				<xsl:choose>
	...
	<xsl:template match="level | level1 | level2 | level3 | level4 | level5 | level6">
		<xsl:element name="{local-name()}" namespace="http://www.daisy.org/z3986/2005/dtbook/">
			...
			<xsl:apply-templates/>
			<xsl:if
				test="count(*[name() != 'hd' and name() != 'h1' and name() != 'h2' and name() != 'h3' and name() != 'h4' and name() != 'h5' and name() != 'h6']) = 0">
				<p/>
			</xsl:if>
		</xsl:element>
	</xsl:template>
